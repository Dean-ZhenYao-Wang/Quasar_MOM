...
/// <typeparam name="TSource">父级，需要继承自BaseEntity</typeparam>
/// <typeparam name="TTarget">子级，需要继承自BaseEntity</typeparam>
public abstract class HierarchyScopeRelationship<TSource, TTarget> : BasicRelationship, IEquatable<BasicRelationship>
    where TSource : class
    where TTarget : HierarchyScope
{
    public HierarchyScopeRelationship()
    { }
    ...
    /// <param name="source">作为关系来源的数字孪生</param>
    /// <param name="target">作为关系目标的数字孪生</param>
    protected void InitializeFromTwins(HierarchyScope source, TTarget target)
    {
        SourceId = source.DtId;
        TargetId = target.DtId;
        Target = target;
    ...
    /// <param name="source">作为关系来源的数字孪生</param>
    /// <param name="target">作为关系目标的数字孪生</param>
    public HierarchyScopeRelationship(HierarchyScope source, TTarget target)
    {
        InitializeFromTwins(source, target);
    }
    ...
    /// <param name="sourceId">源数字孪生唯一标识符</param>
    /// <param name="targetId">目标数字孪生唯一标识符</param>
    public HierarchyScopeRelationship(Guid sourceId, Guid targetId)
    {
        InitializeFromTwins(sourceId, targetId);
    }
    ...
    /// <param name="targetDtId">目标数字孪生唯一标识符</param>
    /// <param name="depth">深度</param>
    public HierarchyScopeRelationship(Guid sourceId, Guid targetId, int depth)
    {
        InitializeFromTwins(sourceId, targetId);
        this.Depth = depth;
    }
    ...
    /// <param name="target">目标数字孪生唯一标识符</param>
    /// <param name="depth">深度</param>
    public HierarchyScopeRelationship(Guid sourceId, TTarget target, int depth) : this(sourceId, target.DtId, depth)
    {
        this.Target = target;
    }
}
